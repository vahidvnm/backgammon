...
class GameViewModel(app:Application):AndroidViewModel(app) {
 ...
 fun start(m:Mode){mode=m;_match.value=MatchInfo();_game.value=TurnState();_screen.value=Screen.GAME}
 fun menu(){_screen.value=Screen.MENU};fun restart(){_game.value=TurnState()};fun update(s:Settings){_settings.value=s;prefs.save(s)}
 ...
 fun move(m:Move){if(m !in GameEngine.legalMoves(_game.value.position,_game.value.dice))return;_game.value=GameEngine.afterMove(_game.value,m);sound();if(_game.value.winner!=null)scoreGame();else autoEnd()}
